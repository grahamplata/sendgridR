...
sendgrid <- function(api, to, subject, from, text,
                     html = NULL, cc = NULL, bcc = NULL, reply_to = NULL, attachment = NULL, filename = NULL) {
  ...
  json <- paste0(json, ', "attachments": [ { "type": "text/csv", "content": "', attachment, '", "filename": "', filename, '" }] }')

  curl_args <- paste0('-X "POST" "https://api.sendgrid.com/v3/mail/send" -H "Authorization: Bearer ', api, '" -H "Content-Type: application/json" -d ')
